...
import kitchenListStyle from "./styles/kitchenListStyle";

const kitchensQuery = {
  path: "kitchens",
};

const useStyles = createUseStyles(kitchenListStyle);
...
function KitchenList({ history }) {
  ...
  if (isEmpty(kitchens)) {
    return <div className={classes.kitchenList}>{"Kitchen list is empty"}</div>;
  }

  return (
    <div className={classes.kitchenList}>
      {kitchens.map(({ value: kitchen, key }, ind) => (
        <KitchenCard
          key={`${key}-${ind}`}
          uid={key}
          history={history}
